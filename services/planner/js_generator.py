...
logger = logging.getLogger("planner.js_gen")

_FILE_HEADER = """'use strict';
...
def _build_test_block(planner_output: dict, description: str, method_name: str) -> str:
    ...
    param_accesses = [f"data.{p['name']}" for p in params]
    ...
    return (
        f"\n"
        f"  const cases_{method_name} = JsonDataProvider.getData('{method_name}');\n"
        f"  test.each(cases_{method_name}.map((d, i) => [i, d]))(\n"
        f"    '{method_name} [%i]',\n"
        f"    async (_i, data) => {{\n"
        f"{low_conf}"
        f"{chain}\n"
...
def generate(planner_output: dict, app_id: str, framework_dir: str,
    ...
    file_path = os.path.join(tests_dir, f"{class_name}.test.js")
    ...
    if os.path.exists(file_path):
        ...
        with open(file_path, "w") as f:
            f.write(content)
        logger.info(f"Created {class_name}.test.js")
