...
for %%S in (dashboard crawl generator planner executor) do (
    echo   Installing %%S...
...
start "Dashboard :8000" cmd /k "cd /d %ROOT%\services\dashboard && poetry run uvicorn main:app --port 8000"
timeout /t 2 /nobreak >nul

echo Starting Executor Service (port 8004)...
start "Executor :8004" cmd /k "cd /d %ROOT%\services\executor && set SHARED_DIR=%SHARED_DIR%&& set JAVA_DIR=%JAVA_DIR%&& set DASHBOARD_URL=%DASHBOARD_URL%&& poetry run uvicorn main:app --port 8004"

echo Starting Crawl Service (port 8001)...
start "Crawl :8001" cmd /k "cd /d %ROOT%\services\crawl && set SHARED_DIR=%SHARED_DIR%&& set DASHBOARD_URL=%DASHBOARD_URL%&& set GENERATOR_URL=%GENERATOR_URL%&& poetry run uvicorn main:app --port 8001"

echo Starting Generator Service (port 8002)...
start "Generator :8002" cmd /k "cd /d %ROOT%\services\generator && set SHARED_DIR=%SHARED_DIR%&& set JAVA_DIR=%JAVA_DIR%&& set DASHBOARD_URL=%DASHBOARD_URL%&& poetry run uvicorn main:app --port 8002"

echo Starting Planner Service (port 8003)...
start "Planner :8003" cmd /k "cd /d %ROOT%\services\planner && set SHARED_DIR=%SHARED_DIR%&& set JAVA_DIR=%JAVA_DIR%&& set DASHBOARD_URL=%DASHBOARD_URL%&& poetry run uvicorn main:app --port 8003"
